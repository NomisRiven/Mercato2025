...
# Chargez les données spécifiques à la Jungle
df_jgl = pd.read_csv("data/Jungle.csv")
...
filters = {}
for column in df_jgl.columns:
    if pd.api.types.is_numeric_dtype(df_jgl[column]):
        filters[column] = create_numeric_filter(df_jgl, column)
    else:
        filters[column] = create_categorical_filter(df_jgl, column)

# Appliquez les filtres
for column, filter_value in filters.items():
    if isinstance(filter_value, tuple):  # Pour les filtres numériques
        df_jgl = df_jgl[(df_jgl[column] >= filter_value[0]) & (df_jgl[column] <= filter_value[1])]
    elif isinstance(filter_value, list) and 'Tous' not in filter_value:  # Pour les filtres catégoriels
        df_jgl = df_jgl[df_jgl[column].isin(filter_value)]
...
if remove_nan:
    df_jgl = df_jgl.dropna()

# Affichez les données filtrées
st.dataframe(df_jgl)

# Ajoutez d'autres éléments spécifiques à la Jungle
# Par exemple :
# st.line_chart(df_jgl["some_column"])